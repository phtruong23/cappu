...
class taxonomy_model(object):
	def __init__(self,
		...
		self.vgg_input_from_resnet = vgg_input_from_resnet
		self.vgg_fully_connected_num = vgg_fully_connected_num
		...
		with slim.arg_scope(partial_vgg.vgg_arg_scope()):
			...
			with tf.variable_scope(scope, 'vgg_19', [vgg19_partial]) as sc:
				...
				if extra_global_features is not None:
					resnet_endpoint_for_vgg_conv_1 = slim.conv2d(end_global, 1024, [3, 3], padding='VALID', scope='resnet_extra_to_vgg_conv_1')
					resnet_endpoint_for_vgg_conv_2 = slim.conv2d(resnet_endpoint_for_vgg_conv_1, 512, [3, 3], padding='VALID', scope='resnet_extra_to_vgg_conv_2')
				...
				# net = slim.conv2d(vgg_out, num_filters, [1, 1], padding='VALID', scope='fc6')
	...
	def build_model(self):
		...
		with tf.variable_scope('resnet_endpart') as sc:
			if self.vgg_input_from_resnet == 'mid':
				self.stage_inputs.append(self.resnet_partial_mid)
			elif self.vgg_input_from_resnet == 'end':
				self.resnet_partial = slim.conv2d(self.resnet_partial, 1024, [3, 3], padding='SAME', scope='resnet_to_vgg_conv_1')
				# self.resnet_partial = slim.conv2d(self.resnet_partial, 1024, [3, 3], padding='VALID', scope='resnet_to_vgg_conv_1')
				# self.resnet_partial = slim.conv2d(self.resnet_partial, 512, [3, 3], padding='VALID', scope='resnet_to_vgg_conv_2')
				self.stage_inputs.append(self.resnet_partial)
				...
				if i < (self.taxonomy_nums - 1):
					with tf.variable_scope((('stage_%d') % i)) as sc:
						if self.vgg_input_from_resnet == 'mid':
							...
							next_input_from_global = slim.conv2d(self.resnet_partial, 512, [1, 1], scope='trans_fc')
	...
	def get_summary_op_val(self):

		summaries = set(tf.get_collection(tf.GraphKeys.SUMMARIES))

		for name in self.eval_updates.keys():
			for stage in self.eval_updates[name].keys():
				summaries.add(tf.summary.scalar('metrics_val/%s/%s' % (name, stage), self.eval_updates[name][stage]))

		for name in self.net_losses.keys():
			summaries.add(tf.summary.scalar('losses_val/%s' % (name), self.net_losses[name]))
