...
def train():
	...
	resnet_loss = get_loss(resnet_partial, batch_label[:, 4])

	resnet_exclude_scopes = 'resnet_v2_50/logits'

	resnet_restore = handle_network_function._get_init_fn('./resnet_v2_50_pretrained/resnet_v2_50.ckpt',
	                                                      '', resnet_exclude_scopes)

	global_step = tf.train.create_global_step()

	update_resnet, learning_rate = set_optimizer(resnet_exclude_scopes, resnet_loss, global_step, len(grasp_loader.train_info))

	eval_values, eval_updates, reset_op = get_metrics(batch_label, resnet_partial)

	resnet_summary = get_summary_op(eval_updates, resnet_loss, learning_rate)

	resnet_val_summary = get_summary_op_val(eval_updates, resnet_loss)
	...
	with tf.Session(config=config) as sess:
		...
		folder_log = './' + 'resnet_train_%s_%s_%s_%s_%s' % (now.year, now.month, now.day, now.hour, now.minute)
		...
		for epoch in range(PARAMS.epochs):
			...
			while (True):
				try:
					...
					update = sess.run({'batch_image': batch_image, 'batch_label': batch_label,
					                   'resnet_partial_mid': resnet_partial_mid, 'resnet_partial': resnet_partial,
					                   'resnet_loss': resnet_loss, 'update_resnet': update_resnet,
					                   'eval_update': eval_updates, 'summary': resnet_summary
					                   })

					print('resnet loss : ', update['resnet_loss'])
			...
			checkpoint_file = os.path.join(folder_log, 'resnet.ckpt')
			...
			while (True):
				try:
					...
					update = sess.run({'batch_image': batch_image, 'batch_label': batch_label,
					                   ...
					                   'eval_update': eval_updates, 'summary': resnet_val_summary
					                   })
